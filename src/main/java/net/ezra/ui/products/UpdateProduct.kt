...
fun UpdateProductScreen(navController: NavController, productId: String, productViewModel: ProductViewModel) {
    ...
    LaunchedEffect(productState) {
        productState?.let {
            updatedProductName = it.name
            updatedProductDescription = it.description
            updatedProductPrice = it.price.toString()
            updatedProductImageUri = if (it.imageUrl.isNotBlank()) Uri.parse(it.imageUrl) else null
        }
    }
    ...
    Column(modifier = Modifier.padding(16.dp)) {
        ...
        Button(onClick = {
            val updatedProduct = Product(
                ...
                price = updatedProductPrice.toDouble().toString(),
                imageUrl = ""
            )
